...
def check_keydown_events(event, ai_settings, screen, ship, bullets):
    ...
    if event.key == pygame.K_RIGHT:
        ...
        #Shortcut to close the game
        sys.exit()

def check_keyup_events(event, ship):
    """Respond to key release"""
    if event.key == pygame.K_RIGHT:
        # Stop right motion of the ship
        ship.moving_right = False
    elif event.key == pygame.K_LEFT:
         # Stop left motion of the ship
...
def check_bullet_alien_collision(ai_settings, screen, stats, sb, ship,
    ...
    if len(aliens) == 0:
        # Destroy existing bullet, speed up game and create a new fleet
        bullets.empty()
        ai_settings.increase_speed()
        create_fleet(ai_settings, screen, ship, aliens)

def fire_bullet(ai_settings, screen, ship , bullets):
    """Fire a bullet if limin not reached yet"""
    # Create a new bullet and add it to the bullet group
...
def create_fleet(ai_settings, screen, ship, aliens):
    ...
    for row_number in range(number_rows):
        for alien_number in range(number_aliens_x):
            # Create an aline and put it in a row.
...
def check_fleet_edges(ai_settings, aliens):
    """Respond appropiately if any aliens have reached the adges"""
...
def ship_hit(ai_settings, stats, screen, ship, aliens, bullets):
    ...
    if stats.ships_left > 0:
        ...
        # Create a new fleet and center the the ship
        create_fleet(ai_settings, screen, ship, aliens)
        ship.center_ship()
